...
module.exports = {
    ...
    checkout: async (req, res) => {
        try {
            ...
            const transaksi = new Transaksi(payload)
    ...
    history: async (req, res) => {
        try {
            ...
            // function $regex dan $options sama seperti like di sql
            if(status.length)
            {
                keriteria = {
                    ...keriteria,
            ...
            const history = await Transaksi.find(keriteria)

            let total = await Transaksi.aggregate([
                {
                    $match : keriteria
                },
                {
    ...
    historyDetail: async (req, res) => {
        try {
            const {id} = req.params
            const history = await Transaksi.findOne({_id:id})
    ...
    dashboard: async (req, res) => {
        try {
            const count = await Transaksi.aggregate([{
            ...
            const history = await Transaksi.find({ player: req.player._id })
                    .populate('category')
                    .sort({'updatedAt': -1})
    ...
    editProfile: async (req, res) => {
        try {
            ...
            {
                const player = await Player.findByIdAndUpdate({ _id: req.player._id },
